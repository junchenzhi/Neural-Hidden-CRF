...
def gen_mv_labels(crowd_labels, O_id=0, T=2):
    worker_numbers = len(crowd_labels[0][0])
    mv_labels = []
    for sentence_crowd_labels in crowd_labels:  # data�?对一�?句子的crowd标�??
        sentence_len = len(sentence_crowd_labels)
        sentence_mv = []
        for i in range(sentence_len):
            label = ""
            word_crowd_labels = sentence_crowd_labels[i]
            word_crowd_dict = dict(Counter(word_crowd_labels))
            if O_id not in word_crowd_dict or word_crowd_dict[O_id] <= worker_numbers - T:
                word_crowd_dict[O_id] = 0
                word_crowd_sort = sorted(word_crowd_dict.items(), key=lambda x: x[1], reverse=True)
                max_num = word_crowd_sort[0][1]
                j = 0
                while word_crowd_sort[j][1] == max_num:
                    j = j + 1
                label = word_crowd_sort[random.randint(0, j - 1)][0]
            else:
                label = O_id
            sentence_mv.append(label)
        mv_labels.append(sentence_mv)
    return mv_labels
...
#         word = split_line[0]
#         embedding.append(np.array([float(val) for val in split_line[1:]]))
#         # if np.array([float(val) for val in split_line[1:]]).shape[0] != 100:
#         #     print(np.array([float(val) for val in split_line[1:]]).shape[0])
#         #     print('exit!!!!!!!!!!!!!!!!!')
#         #     print(i)
#         #     exit()
#         word_dict[word] = i
...
#     '''
#     print('\n')
#     print("embedding[0].shape:", embedding[0].shape)
#     print("embedding[1].shape:", embedding[1].shape)
#     print("embedding[2].shape:", embedding[2].shape)
#     print('\n')
#     print('\n')
...
#     word_dict[UNK] = len(word_dict)
#
#     # embedding = embedding.reshape(embedding.shape[0], embedding[0].shape[0])
#
#     # �����������ĸı�
#     # dict_len, embed_size = embedding.shape
#     embed_size = 100
